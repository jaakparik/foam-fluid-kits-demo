...
import { PitchKitThumbnail } from "@/components/PitchKitThumbnail";
import type { PitchKitData } from "@/data/pitchKits";

interface PitchKitCardProps {
  kit: PitchKitData;
  index: number;
  customImages?: string[];
}

export function PitchKitCard({ kit, index, customImages }: PitchKitCardProps) {
  return (
    <div className="flex cursor-pointer flex-col gap-2 overflow-hidden rounded-xl border border-border bg-card p-3">
      ...
      <div className="relative flex h-[140px] w-full items-center justify-center overflow-hidden rounded-lg">
        <PitchKitThumbnail seed={index} customImages={customImages} />
      </div>
